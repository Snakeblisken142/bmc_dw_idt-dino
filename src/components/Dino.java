...
public class Dino {
  ...
  private static int jumpFactor = 20; //speed. 1000 will make him to go to space

  public static final int STAND_STILL = 1,
                    RUNNING = 2,
                    JUMPING = 3,
                    DIE = 4;
  private final int LEFT_FOOT = 1, //the state #'s are important. the foot are... just there.
                    RIGHT_FOOT = 2,
                    NO_FOOT = 3;
  ...
  BufferedImage rightFootDino; //we love screen flickering
  ...
  public Dino() {
    ...
    dinoBaseY = Ground.GROUND_Y + 10;
    dinoTopY = Ground.GROUND_Y - image.getHeight() + 10; //on ground
    dinoStartX = 10;
    dinoEndX = dinoStartX + image.getWidth();
    topPoint = dinoTopY - 120; //jump 
  ...
  public void create(Graphics g) {
    ...
    switch(state) {
      ...
      case STAND_STILL:
        System.out.println("ALRIGHT WE'RE IN");
      ...
      case RUNNING:
        if(foot == NO_FOOT) {
          ...
          foot = RIGHT_FOOT;
          g.drawImage(rightFootDino, dinoStartX, dinoTopY, null); //running animation
        } else {
          foot = LEFT_FOOT;
          g.drawImage(leftFootDino, dinoStartX, dinoTopY, null);
        }
      ...
      case JUMPING:
        ...
        if(dinoTop >= topPoint && !topPointReached) {
          topPointReached = true; //change this rq
          g.drawImage(image, dinoStartX, dinoTop += jumpFactor, null);
          break;
        }         
        if(dinoTop > topPoint && topPointReached) {      
          if(dinoTopY == dinoTop && topPointReached) {
            state = RUNNING;
            topPointReached = false;                    //prevents deaddino midjump.
            break;
          }    
          g.drawImage(image, dinoStartX, dinoTop += jumpFactor, null);          
          break;
  ...
  public void jump() {
    dinoTop = dinoTopY;
    topPointReached = false; //true = no jump.
    state = JUMPING;
  }
}